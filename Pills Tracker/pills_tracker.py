...
main_kb = ReplyKeyboardMarkup(
    [["Добавить новый препарат"], ["Я ем"], ["В главное меню"]],
    resize_keyboard=True,
    input_field_placeholder="Воспользуйтесь меню:",
    one_time_keyboard=True,
)
# обычная да/нет клавиатура
...
async def handle_newmed(update, context):  # запускаем добавление лекарства
...
async def handle_food(update, context):  # запускаем выдачу информации
...
async def handle_text(
    ...
    if state:  # ждем, пока у нас не state, пока мы нигде
        if state["step"] == "ask_new_medicine":
        ...
        elif state["step"] == "ask_dose":  # спросили и спрашиваем дозу
            medicine_name = state["current_medicine"]
            state["medicines"][medicine_name].append(text)
            state["step"] = "ask_longitude"
            await update.message.reply_text(
                "Сколько дней курс приема (введите целым числом)?\nВведите 1000, если Вы принимаете препарат регулярно:"
            )

        elif state["step"] == "ask_longitude":  # спрашиваем продолжительность приема
            if not text.isdigit():
                return await update.message.reply_text("Введите количество дней числом")
            medicine_name = state["current_medicine"]
            state["medicines"][medicine_name].append(int(text))
            ...
            if text == "Завтракаю":
                    ...
                    if "Завтрак" in value:
                        if "До еды" in value:
                            printed["Выпить до еды"].append(
                                f"{key} по {value[0]} за {value[-1]} до еды"
                            )
                            ...
                            printed["Выпить после еды"].append(f"{key} по {value[0]}")
                ...
                for key, value in state["medicines"].items():
                    if "Обед" in value:
                        if "До еды" in value:
                            printed["Выпить до еды"].append(f"{key} по {value[0]}")
                        elif "Во время еды" in value:
                            printed["Выпить во время еды"].append(
                                f"{key} по {value[0]}"
                            )
...
def main():
    ...
    application = (
        Application.builder()
        .token("7560481764:AAG44y5V-9GHO-fX8ZN05Xafo8jciiPKUug")
        .build()
    )
    application.add_handler(CommandHandler("nachalo", start_med))
