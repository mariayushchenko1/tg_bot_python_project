...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Зарядка":
        ...
        await update.message.reply_text("Вы в трекере сна", reply_markup=SLEEP_KEYBOARD)
    elif text == "Трекер настроения":
        await update.message.reply_text(
            "Вы в трекере настроения", reply_markup=MOOD_KEYBOARD
        )
    elif text == "Водный баланс":
        await update.message.reply_text(
            "Вы в разделе водного баланса", reply_markup=WATER_KEYBOARD
        )
    elif text == "Трекер приема таблеток":
        await update.message.reply_text(
            "Вы в трекере приема таблеток", reply_markup=PILLS_KEYBOARD
        )
    elif text == "Назад":
