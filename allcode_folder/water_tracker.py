...
# мы умные и не загружаем сам токен в открытый репозиторий

bot = Bot(token="ВСТАВИТЬ СЮДА ТОКЕН")
dp = Dispatcher()
...
# прозрачные кнопки (кнопка для того, чтоб отметить выпитый стакан
# + клавиатура для частоты напоминаний)


def glass_buttons():
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🥤 Стакан выпит", callback_data="drink")]
        ],
    )
...
restart_keyboard = ReplyKeyboardMarkup(
    keyboard=[[KeyboardButton(text="🔄 Вернуться к началу")]], resize_keyboard=True
)
...
async def start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "💧 Привет!\nДля начала мне нужно рассчитать дневную норму воды. Пожалуйста, "
        "введите свой вес"
        "(в кг):\n\n(Если хотите вернуться к выбору разделов, перейдите в @SaludHealthCareBot)",
...
async def get_weight(message: types.Message, state: FSMContext):
    try:
        ...
        await state.update_data(weight=weight, norm=calculate_norm(weight), drunk=0)
...
async def get_start_time(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            "Неверный формат — пожалуйста, введите время в формате ЧЧ:MM"
        )
...
async def get_end_time(message: types.Message, state: FSMContext):
    try:
        ...
        task = asyncio.create_task(
            schedule_reminders(message.from_user.id, user_data["frequency"])
        )
        user_timers[message.from_user.id] = task

        await message.answer(
            f"✅ Настройки сохранены!\n"
            f"• Дневная норма (в стаканах): {user_data['norm']} \n"
        ...
        try:
            ...
            if remaining <= 0:
                await bot.send_message(
                    user_id, "🎉 Ура, дневная норма выполнена!\nДо завтра"
                )
                break

            await bot.send_message(
                user_id,
                f"⏰ Пора пить воду! Осталось стаканов: {remaining}",
...
@dp.message(lambda message: message.text == "🔄 Вернуться к началу")
    ...
    for user_id in list(current_users.keys()):
        try:
            ...
            if user_id in user_timers:
                user_timers[user_id].cancel()
            task = asyncio.create_task(
                schedule_reminders(user_id, current_users[user_id]["frequency"])
            )
